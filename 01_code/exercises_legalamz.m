...
if i ==1
    protected_areas = readgeoraster("protected.tif");
    productivity = readgeoraster("productivity.tif");
    deforestation = readgeoraster("deforestation.tif");
    ...
    F = 3.5*ones(params.N);               % Homogeneous deforestation cost
                                          % target size
    L = zeros(params.N, params.N, 'like', protected_areas);                  % padded output (zeros elsewhere)
    L(1:m, 1:n) = protected_areas;

    D = zeros(params.N, params.N, params.Tmax, 'like', deforestation);                  % padded output (zeros elsewhere)
    ...
    ret = SolveDeforestationAMZ(params,A,F,O,D,L,Agribusiness,O_aux,1,false,mask);
end
